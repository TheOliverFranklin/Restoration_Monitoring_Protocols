...
The first step is to combine all of the years and/or sites of LWD data that you wish to compare into one dataframe. Note that, if you also collected logjam data, that should be kept as separate files, as we will deal with that data in a separate script.
...
data_dir <- "C:/Users/franklino/Documents/Restoration_Monitoring_Protocols/data_raw/Black Creek/LWD/to_combine"
```
...
df <- df %>%
 ...
 # Rootwad: default to "N" for NA, blank, or "-", then parse based on presence of 'p' or 'a'
    Rootwad = case_when(
      ...
      str_detect(str_to_lower(Rootwad), "^(p|y|yes|present)") ~ "Y",
      str_detect(str_to_lower(Rootwad), "^(a|n|no|absent)") ~ "N",
      TRUE ~ Rootwad
    )
  )
...
if (interactive()) {
  ...
  output_folder <- "C:/Users/ZARGARPOURN/Documents/SAR Team/Monitoring Protocols/R Code/LWD/Output"
  if (!dir.exists(output_folder)) {
    dir.create(output_folder, recursive = TRUE)
