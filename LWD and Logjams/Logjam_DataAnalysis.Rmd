...
Before we dive into the analyses, let's briefly recall the survey methodology:
...
Logjam locations are shown in the interactive map below, coloured by year and with symbol sizes indicative of relative logjam area (width x length as measured in field). You can toggle between basemaps and select the sites/survey year(s) of interest. Solid circular points (coloured according to survey year) indicate logjam locations and are labelled with site, year, Jam ID, and area (clicka a point to see its label). Magenta X symbols indicate the start and end points of the focal reach(es).

Note that the circle size indicates relative area among logjams by using the square-roots of each area (to account for the potential large range in logjam sizes). The size of the circle does not indicate the actual size of the logjam on the map. Also bear in mind that the area calculation is based on a square that would contain the logjam's footprint - it does not mean that the full area is comprised of wood, nor does it contain any height information. But with this in mind, we feel that a simple visual representation on the map of both location and size can be very helpful in understanding how logjams move and change over time.
...
__Total Large Wood in the Reach__

You may be interested in how much large wood moves into and out of your study reach, and/or how wood moves within the reach. Logjams are one component of large wood in the reach, and so we imported data from the LWD script so that we can better understand to what degree observed changes suggest movement within versus movement into and out of your reach. The table above includes the numbers of LWD pieces within logjams, outside of logjams, and the proportion of the total large wood in your reach found within logjams.

We will plot the proportion of LWD that is found within logjams below, and export it to your output folder. You must consider this figure alongside the (non-logjam) LWD Volume figure from the LWD Data Analysis script, and the inferential test in that script. It could lead to avoidable errors if you interpret changes in non-logjam LWD volume without an understanding of whether LWD has been moving into or out of logjams. 
...
logjam_summary <- logjam_data %>%
  ...
  summarise(
    ...
    Total_LWD_Independent = first(Total_LWD_Piece),  # Total independent LWD
    .groups = "drop"
  ) %>%
  mutate(
...
### Total Large Wood (this title is repeating the above... reorganise)

We felt that including logjam porosity (wood-to-air/water ratio) variability into a volume estimation would not be sufficiently repeatable, and that measuring every piece of LWD in a logjam would be prohibitively time-consuming in the field for many salmon watersheds. As such, we lack a measurement of volume that is comparable between the non-logjam LWD data and the logjam data. Instead, we opt to examine how the total frequency of large wood in the reach (individual pieces plus those in logjams) changes over time and between sites. While this does sacrifice some information (volume variaiblity among LWD pieces) it retains repeatability and objectivity, which take precedence when faced with the reality of different individuals taking measurements over the years. 

Although we do not conduct inferential tests on volume, your understanding of how wood changed within your reach can be based on observations such as the following:
...
While admittedly requiring a bit more head-scratching, we think that the above is the most appropriate and informative approach to gain understanding of the various nuances involved, within the logistical constraints of the typical restoration resources available. As such, we do not provide the tempting metric of 'total reach wood volume' by projecting mean LWD volume onto the frequencies of wood in the logjams. To do so would be to assume that volume distributions of LWD pieces in a logjam would be the same as for those outside of logjams. We suspect that this is untrue, but our field protocol still collected sufficient data to permit calculation of this metric should you feel otherwise. But bear in mind, even with this approach, you would not have individual-level data necessary for an inferential test.

Here we provide plots that can be used to examine the total frequency of large wood within your reaches (logjam plus independent LWD pieces) over time.

### Visualisation

#### Before / After Years

Enter in the below code the years that you consider 'before' and 'after' your restoration action. Ideally these should balance (equal number of years before as after), but they need not be consecutive years. The purpose of this categorisation is so that, if you have multiple years of data before and after your restoration actions, we can visually compare the slopes of the lines that fit within each category. This is to say, we can see if the rate of change of large wood frequency differs since the restoration action, which might help tease apart effects of your restoration from background effects (especially since we do not have suitable data for inferential testing). Note that if you do not have 2 or more years in each before and after category, a line will be fit across all the years together.
...
```{r total large wood visualisation, echo = FALSE}
# COMPUTE TOTAL LWD PER REACH
df <- summary_table_jam %>%
  mutate(
    Total_Reach_LWD = LWD_Nr_Logjam + LWD_Nr_Free,
    year = as.numeric(year)   # ensure continuous scale
  )

#3. CHECK WHETHER TO SPLIT INTO BEFORE/AFTER SEGMENTS
...
if (do_piecewise) {
  ...
  # PLOT: Separate dashed/solid lines by Period
ggplot(df, aes(x = year, y = Total_Reach_LWD, color = Site)) +
    geom_point(size = 3) +
    geom_line(aes(group = SitePeriod, linetype = Period), linewidth = 1) +
    ...
    labs(
      ...
      color    = "Site",
      linetype = "Period",
      title    = "Piecewise LWD Trajectories: Before vs After"
    ) +
    theme_minimal(base_size = 14)

} else {
  # SIMPLE: One line per Site across all years
 ggplot(df, aes(x = year, y = Total_Reach_LWD, color = Site, group = Site)) +
    geom_point(size = 3) +
