...
In general, your data should be able to differentiate where you surveyed but did not encounter a variable (e.g., looked for fine sediment, but none was present) versus where you did not survey (e.g., did not look for fine sediment). If you did survey fine sediment, please ensure there is at least one value (Y or N) entered within your data, so that our code knows that a survey was conducted.

In your dataframe you may have columns that include notes (e.g. aquatic vegetation) which will need to be summarised or categorised before analysis. If applicable, take the time to **do this now**. It is probably simplest to summarise this data in familiar spreadsheet software, like excel. For example, if you were recording aquatic vegetation, add columns 'subveg', 'emergveg', 'floatveg', in which you will enter the proportion (0 to 1; 2dp) of the wetted width that was recorded as containing submergent, emergent, and floating aquatic vegetation.

Depending on the project objectives and the data collected, it may be useful to go further and (e.g.) explore aquatic vegetation by species or status (invasive/exotic, hydrophyte etc.). To ensure consistency with species names, we recommend using BC plant codes, available from [BC Species Explorer](https://www.for.gov.bc.ca/hre/becweb/resources/codes-standards/standards-species.html). Invasive status and Hydrophyte status should be based on local knowledge if you are familiar with the species and the standard definitions for 'hydrophyte', 'obligate', 'facultative (wetland)' etc. If you are unsure of hydrophytic status, you can look up plants at [USDS Plant Database](https://plants.usda.gov/home):
enter plant name at the search box in the top left. Once you are at the plant's page, select the 'wetland' tab, and read wetland status based on most appropriate Region for you (e.g., Alaska or Western Mountains, Valleys, and Coast).
