...
If your first pool in the sequence is not deemed to be meaningful or repeatable (i.e., an artificial pool outlet was generated and you feel you will be unable to repeat surveys from a consistent location), there is an option here to remove the first pool entirely. Note that if you omit pool 1 here, it will be omitted from all summary stats and inferential tests going forward (including for fines & aquatic veg).

```{r omit first pool}
# --- User option: include or omit pool 1 ---
Pool1Omit <- FALSE   # set to TRUE to exclude pool 1
```
...
calculate_subset_summary <- function(pool_summary_df, residuals.df, Pool1Omit = FALSE) {
  # If Pool1Omit is TRUE, filter out pool_id == 1
  if (Pool1Omit) {
    pool_summary_df <- pool_summary_df[pool_summary_df$pool_id != 1, ]
  }
...
subset_summary_result <- calculate_subset_summary(pool_summary_result, residuals.df, Pool1Omit)
