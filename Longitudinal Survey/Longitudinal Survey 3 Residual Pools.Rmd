...
Enter your decision to omit or include the first pool in the chunk of code below. Note that you must follow Steps 1 and 2 in the below code if you have only one year of data, or if your decision regarding Pool 1 was consistent across all sites and years that will be analysed today. If your decision for that first pool was not consistent across all sites and years, complete Steps 1 and 3 in the below code. Our default, if you alter nothing, will be to keep the first pool in the analyses.
...
consistent_pool1_rule <- FALSE   # If this remains 'TRUE', go to Step 2. 
...
before_years <- c(2025) #add more years inside brackets if needed, e.g. 'c(2024, 2025, 2026)'
...
```
# up to here. adding the pool1omit logic. Takes time. need to also look through scripts 4 and 5 in case it causes issue
...
```{r fines, echo = FALSE}
calculate_fines_proportion <- function(pool_data, residuals.df, custom_depth_threshold = NULL, use_sd_threshold = FALSE, Pool1Omit = FALSE) {
...
fines_proportion_result <- fines_proportion_result %>%
 { 
    if (exists("Pool1Omit") && isTRUE(Pool1Omit)) {
      filter(., pool_id != 1)
    } else {
      .
    }
...
calculate_veg_proportion <- function(residuals.df, veg_column, custom_depth = NULL, Pool1Omit = FALSE) {  # Validate that the vegetation column exists in the residuals dataframe
...
for (i in 1:nrow(unique_sites_years)) {
  ...
  if (nrow(subset_residuals) == 0) {
    ...
    next
      ...
      # Drop pool 1 if Pool1Omit is TRUE
    if (isTRUE(Pool1Omit)) {
      subset_residuals <- subset_residuals[subset_residuals$pool_id != 1, ]
    }
...
subveg_result <- calculate_veg_proportion(residuals.df, veg_column = "subveg", custom_depth = custom_depth, Pool1Omit = Pool1Omit)
emergveg_result <- calculate_veg_proportion(residuals.df, veg_column = "emergveg", custom_depth = custom_depth, Pool1Omit = Pool1Omit)
floatveg_result <- calculate_veg_proportion(residuals.df, veg_column = "floatveg", custom_depth = custom_depth, Pool1Omit = Pool1Omit)
