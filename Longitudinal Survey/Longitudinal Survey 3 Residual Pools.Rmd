...
calculate_residual_depths <- function(df) {
  ...
  if (residuals.df$distance[1] < 0) {
    ...
    for (e in 1:nrow(residuals.df)) {
     ...
     # fix for residual surface becoming NA at certain LDMs
    # This enables the LDM loop to continue projecting from a valid surface.
    seed_end_index <- suppressWarnings(max(which(!is.na(residuals.df$residual_surface))))
    if (is.finite(seed_end_index)) {
      last_residual_surface <- residuals.df$residual_surface[seed_end_index]
      last_residual_index   <- seed_end_index
    } else if (!is.na(downstream_depth)) {
      # Fallback: if Stage‑A wrote nothing but the first row has a valid depth, seed from there.
      last_residual_surface <- downstream_depth
      last_residual_index   <- downstream_index
      residuals.df$residual_surface[downstream_index] <- downstream_depth
      residuals.df$residual_depth[downstream_index]   <- 0
    }
  }
  ...
  for (b in 2:(nrow(residuals.df) - 1)) {
    ...
    if ((residuals.df$Thalweg_Depth_m[b] <= (residuals.df$Thalweg_Depth_m[b - 1] + slope_factor * spacing_prev) &&
      ...
      LDM_id <- LDM_id + 1
      downstream_depth <- residuals.df$Thalweg_Depth_m[b] #projects new resid surface from this LDM
      downstream_index <- b
      ...
      if (is.na(last_residual_surface) || residuals.df$distance[b] > residuals.df$distance[last_residual_index]) {
        ...
        # Guard the comparison to avoid 'missing value where TRUE/FALSE needed'
        if (!is.na(downstream_depth) && !is.na(projected_residual_surface) &&
            downstream_depth <= projected_residual_surface) {
          last_residual_surface <- downstream_depth
          last_residual_index   <- b
        } else {
          next
...
# --- Fallback: turn leftover NA residuals into 0-depth at the thalweg ---
# Yes this is sloppy, but I'm tired. Here we deal with an issue whereby LDMs are needed to project a new residual surface / pool, but in some cases there are no residual surfaces initiated when a pool terminates before an LDM (e.g., lower on a riffle). I cannot figure out the issue and fix elegantly, so based on assumption that this only occurs when the residual surface would continue to project at a lower elevation than the thalweg as we go upstream but do not increase in thalweg depth enough to intersect it. So my sloppy fix is here to assign 0 thalweg depth and residual surface = thalweg depth for all of the NA values (which seem to only occur in this special case)
na_idx <- is.na(residuals.df$residual_depth) & 
          !is.na(residuals.df$Thalweg_Depth_m) & 
          residuals.df$Location_Code != "nickpts"

residuals.df$residual_depth[na_idx]   <- 0
residuals.df$residual_surface[na_idx] <- residuals.df$Thalweg_Depth_m[na_idx]
...
Note that, if you were not able to begin the survey downstream at a pool outlet, an artificial outlet has been generated. You will have the option to omit this entire first pool from subsequent analyses if deemed appropriate.
...
# Repeat palette so it's long enough for all pools (and hopefully still distinguishing among nearby pools)
n_pools <- length(unique(residuals.df$pool_id))
custom_palette <- rep(base_palette, length.out = n_pools)
...
create_pool_summary_table <- function(residuals.df, custom_depth_threshold = NULL) {
  ...
  for (group_name in names(residuals.df_grouped)) {
    ...
    # Only run the loop if the pool has at least 2 rows
    if (nrow(pool_data) >= 2) {
...
pool_summary_result <- pool_summary_result %>%
  dplyr::select(Site, year, everything())  # Move Site and Year to the left
datatable(
  pool_summary_result,
...
We will now generate and export to your output folder some summary statistics, including the aforementioned mean max residual depth and length-in-residual-pool, for your data. But before we look at summary statistics at the pool level, we need to know if you want to include the pool at the very start of your survey.

### Omit First Pool Option

The first pool in the sequence is potentially not meaningful for your study and/or may not be repeatable, for example if you were unable to start your survey at a genuine pool outlet / riffle crest.  In our code we generated an artificial pool outlet so that the initial pool metrics can be calculated anyway, but we recommend only including the artificially-truncated first pool in the following analyses if you are confident you can repeat future surveys at the same consistent location. Note that if you decide to omit the first pool in the following code, it will be omitted from all summary statistics and inferential tests going forward (including for fines & aquatic veg).

If you *are* considering omitting the first pool, take another look at the figure we generated above. In some cases (e.g., a beaver 'involved' with your restoration) you may have started your survey in a pool that backwaters quite far upstream. Omitting this pool might mean losing a considerable amount of data, which complicates comparisons across years that do not feature such pools. So before making the decision to omit any data, consider how this might impact your data and your interpretations, and (probably most important) whether this first pool is actually an important component of the variation that occurs at your sites, or whether it can be considered an artefact that could be omitted. We suggest that, if you are able to repeat your surveys each year at a reasonably consistent location, then including data from the first pool (with the artificial pool outlet generated here) is preferable.

Enter your decision to omit or include the first pool in the chunk of code below. Note that you must follow Steps 1 and 2 in the below code if you have only one year of data, or if your decision regarding Pool 1 was consistent across all sites and years that will be analysed today. If your decision for that first pool was not consistent across all sites and years, complete Steps 1 and 3 in the below code. 

Our default, if you alter nothing, will be to keep the first pool in the analyses. Note that, if you omit the first pool in a given site and year, the statistic 'proportion length in residual pool' will be calculated based on a total reach length that also omits the first pool.


```{r omit first pool}
# --- User input required: Pool 1 inclusion or omission

# ---------- Step 1: change the below 'TRUE' to 'FALSE' if your decision about Pool 1 is not consistent across all sites and years analysed in this session
consistent_pool1_rule <- TRUE   # If this remains 'TRUE', go to Step 2. 
                                # If you enter 'FALSE' go to Step 3.

if (consistent_pool1_rule) {
  global_pool1_omit <- FALSE    # ---------- Step 2: set to TRUE to exclude Pool 1 from analyses (across all sites and years where present),
                                # or set to FALSE to include Pool 1 across the board.
  
  pool1_settings <- residuals.df %>%
    distinct(Site, year) %>%
    mutate(Pool1Omit = global_pool1_omit)
  
} else {
  # ---------- Step 3: List all site-year combinations in the data to be analysed, as well as whether Pool 1 should be omitted (TRUE) or included (FALSE).
  
  pool1_settings <- tibble::tribble(
    ~Site,               ~year,  ~Pool1Omit,
    "Fish Trap Channel", 2025,  TRUE,
    "Fish Trap Channel", 2026,  TRUE,
    "Fish Free Channel", 2025,  FALSE,
    "Fish Free Channel", 2026,  FALSE
    # add more rows for extra Site-years, and don’t forget the comma for the penultimate line
  )
}

pool1_settings<-pool1_settings %>%
  mutate(year = as.character(year))

#df <- df %>%
#  left_join(pool1_settings, by = c("Site", "year")) %>%
#  filter(!(Pool1Omit & pool_id == 1))   # drop pool 1 rows where Pool1Omit == TRUE
```
...
calculate_subset_summary <- function(pool_summary_df, residuals.df, pool1_settings) {
  
   # Join the pool1_settings (Site, year, Pool1Omit) to the pool_summary_df
  pool_summary_df <- pool_summary_df %>%
    left_join(pool1_settings, by = c("Site", "year"))
  
   # Filter out pool_id == 1 only where Pool1Omit == TRUE
  pool_summary_df <- pool_summary_df %>%
    filter(!(Pool1Omit & pool_id == 1)) 
  ...
  for (group_name in names(pool_summary_grouped)) {
    ...
    # check Pool1Omit for this site-year (this would reduce the total reach length denominator to also exclude the first pool)
pool1omit <- unique(group$Pool1Omit)
if (isTRUE(pool1omit)) {
  # find max distance for pool_id == 1 in this site-year
  pool1_max <- max(group_residuals.df$distance[group_residuals.df$pool_id == 1], na.rm = TRUE)
  # subtract it from total reach length
  total_reach_length <- total_reach_length - pool1_max
}
...
# Run with user option on pool 1
subset_summary_result <- calculate_subset_summary(pool_summary_result, residuals.df, pool1_settings)
...
# also export residuals dataframe for later use
...
# Categorize years and incorporate Pool 1 inclusion/omission from above summary stats
pool_summary_result <- pool_summary_result %>%
  left_join(pool1_settings, by = c("Site", "year")) %>%
  # Drop pool 1 only for site-years where Pool1Omit == TRUE
  filter(!(Pool1Omit & pool_id == 1)) %>%
...
calculate_pool_volumes_for_sites_years <- function(df, pool_summary_df, pool1_settings) {
pool_summary_df <- pool_summary_df %>%
    left_join(pool1_settings,by = c("Site", "year"))
  ...
  for (group_name in names(grouped_pool_summary)) {
    ...
    # Apply Pool1Omit rule: drop pool_id == 1 if flagged
    if (isTRUE(pool1omit)) {
      subset_pool_ids <- subset_pool_ids[subset_pool_ids != 1]
    }
 ...
 # Right before building the data frame
vols <- unlist(pool_volumes)

pool1omit <- if (length(pool1omit) == 0) NA else pool1omit[1]

    # Convert the results into a dataframe for the current Site-Year group
n_pools <- length(unlist(pool_volumes))
pool_volumes_df <- data.frame(
  pool_id       = names(pool_volumes),
  pool_volume_m3 = unlist(pool_volumes),
  Site          = rep(site, n_pools),
  year          = rep(year, n_pools),
...
pool_volumes_result <- calculate_pool_volumes_for_sites_years(residuals.df, pool_summary_result, pool1_settings)
...
```{r fines, echo = FALSE}
calculate_fines_proportion <- function(pool_data, residuals.df, pool1_settings, custom_depth_threshold = NULL, use_sd_threshold = FALSE) {
  ...
  for (i in 1:nrow(unique_sites_years)) {
    ...
    # get Pool1Omit flag for this site-year
    pool1omit_flag <- pool1_settings %>%
      dplyr::filter(Site == site, year == year) %>%
      dplyr::pull(Pool1Omit)
    pool1omit_flag <- if (length(pool1omit_flag) == 0) FALSE else pool1omit_flag[1]
    
    # If Pool1Omit = TRUE, drop pool_id == 1
    if (isTRUE(pool1omit_flag)) {
      subset_residuals <- subset_residuals[subset_residuals$pool_id != 1, ]
    }
    ...
    for (pool_id in unique_pool_ids) {
      ...
      for (j in 1:nrow(pool_residuals)) {
        if (j == 1) {
          pool_residuals$segment_length[j] <- (pool_residuals$distance[j + 1] - pool_residuals$distance[j]) / 2
        } else if (j == nrow(pool_residuals)) {
          pool_residuals$segment_length[j] <- (pool_residuals$distance[j] - pool_residuals$distance[j - 1]) / 2
        } else {
          pool_residuals$segment_length[j] <- 
            ((pool_residuals$distance[j] - pool_residuals$distance[j - 1]) / 2) +
            ((pool_residuals$distance[j + 1] - pool_residuals$distance[j]) / 2)
        }
      }
...
## This code does not generate anything yet - it is just a function that will be applied after the custom depth user specificaiton
```
...
#### add the before/after categories and pool1omit value from previous input
...
calculate_veg_proportion <- function(residuals.df, veg_column, pool1_settings, custom_depth = NULL) {  # Validate that the vegetation column exists in the residuals dataframe
...
for (i in 1:nrow(unique_sites_years)) {
 ...
 # Look up Pool1Omit flag for this site-year
    pool1omit_flag <- pool1_settings %>%
      dplyr::filter(Site == site, year == year) %>%
      dplyr::pull(Pool1Omit)
    pool1omit_flag <- if (length(pool1omit_flag) == 0) FALSE else pool1omit_flag[1]
    
    # Drop pool 1 if flagged
    if (isTRUE(pool1omit_flag)) {
      subset_residuals <- subset_residuals[subset_residuals$pool_id != 1, ]
    }
    if (nrow(subset_residuals) == 0) {
      message(paste("All pools omitted for Site:", site, "Year:", year))
      next
    }
  ...
  #cat("Depth threshold for Site:", site, "Year:", year, "is:", depth_threshold, "\n")
    ...
    valid_pools <- suppressWarnings( # suppress warnings as I give up making them coherent - they only appear when no veg in site-year)
    subset_residuals %>%
      group_by(pool_id) %>%
...
# again, nothing exported or displayed... just a funky little function
```
...
subveg_result <- calculate_veg_proportion(residuals.df, veg_column = "subveg", pool1_settings = pool1_settings, custom_depth = custom_depth)
emergveg_result <- calculate_veg_proportion(residuals.df, veg_column = "emergveg", pool1_settings = pool1_settings,custom_depth = custom_depth)
floatveg_result <- calculate_veg_proportion(residuals.df, veg_column = "floatveg", pool1_settings = pool1_settings,custom_depth = custom_depth)
...
run_beta_regression <- function(data, response_var, dataset_name, summary_list) {
  # Create the Time_Category and omit pool 1 if applied above
  data <- data %>%
  mutate(Time_Category = case_when(
    year %in% before_years ~ "Before",
    year %in% after_years ~ "After",
