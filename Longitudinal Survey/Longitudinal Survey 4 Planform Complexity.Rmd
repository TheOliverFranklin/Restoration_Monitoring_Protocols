...
nodes_df <- df %>%
  mutate(Nodes = as.character(Nodes)) %>%        # ensure character
  mutate(Nodes = trimws(Nodes)) %>%              # remove surrounding whitespace
  filter(
    !is.na(Nodes),
    Nodes != "",
    Nodes != "-",
  ...
  mutate(
    Type   = substr(Nodes, 3, 3),
    WetDry = substr(Nodes, 4, 4)
  )
...
rci_results_clean <- rci_results %>%
  ...
  dplyr::select(-stream_length, -Straight_Line_Distance, -Thalweg_Line_Length)  # Exclude the stream_length column (incorrect if no nodes), and others to tidy
