...
Before we generate metrics relating to beaver or debris pools, we need to consider how to treat the first pool. We will repeat the approach used in 'Longitudinal Survey 3 Residual Pools'.

### Omit First Pool Option

Your decision again will depend on the particular circumstances for your site and study, but we again stress the need for careful consideration. If you are able to repeat your surveys each year at a reasonably consistent location, then including data from the first pool (with the artificial pool outlet we generated ) is generally preferable to omitting potentially meaningful data. This is especially relevant here, given that debris jams and beaver dams might often be the very reason that you were unable to begin your survey at a pool outlet.
...
Our default, if you alter nothing, will be to keep the first pool in the analyses.
...
calculate_beaver_and_debris_pools_by_site_year <- function(df, pool1_settings) {
  ...
  for (site_year in names(site_year_groups)) {
    ...
    if (isTRUE(pool1omit_flag)) {
      site_year_data <- site_year_data[site_year_data$pool_id != 1, ]
    }
    if (nrow(site_year_data) == 0) {
      message(sprintf("All pools omitted for Site '%s' Year %s", site, year))
      next
    }
    ...
    for (pool_id in unique_pool_ids) {
     ...
     if (pool_length <= 0) {
     pool_type <- "puddle"
    } else if (is_beaver_pool || has_beaver_outlet) {
    pool_type <- "Beaver Pool"
    } else if (is_debris_pool || has_debris_outlet) {
    pool_type <- "Debris-Dam Pool"
    } else {
     pool_type <- "Other"
}
...
sagittal_area <- 0
if ("residual_depth" %in% names(pool_data)) {
  for (i in seq_len(max(0, nrow(pool_data) - 1))) {
    depth <- pool_data$residual_depth[i]
    if (is.null(depth) || is.na(depth) || depth <= 0) next
    spacing <- abs(pool_data$distance[i + 1] - pool_data$distance[i])
    sagittal_area <- sagittal_area + (depth * spacing)
  }
  
  # Handle last row contribution
  if (nrow(pool_data) > 0) {
    last_row <- pool_data[nrow(pool_data), ]
    if (!is.null(last_row$residual_depth) &&
        !is.na(last_row$residual_depth) &&
        last_row$residual_depth > 0) {
...
# Apply the function
result <- calculate_beaver_and_debris_pools_by_site_year(residuals.df, pool1_settings)
...
Because we are now using field-assigned habitat units, the below metrics are not influenced by your decision above to omit/keep the first pool in the survey. These habitat unit metrics are calculated based on the full length of your survey.
...
plot_data_long_percent <- result %>%
  dplyr::select(Site, year, pool_percent, scourpool_percent, dammedpool_percent) %>%
