...
# Read and combine all CSV files into one dataframe, clean sand, fines, and create new embedded variables
df <- csv_files %>%
  ...
  # Step 1 — standardize 'sand' and 'fines'
  mutate(
    across(starts_with("B_Axis_T"), ~ case_when(
      grepl("^\\d+(\\.\\d+)?$", .) ~ as.character(as.numeric(.)),  
      grepl("sand", ., ignore.case = TRUE) | grepl("^\\s*S\\s*$", ., ignore.case = TRUE) ~ "sand", # match 'sand' or single 'S'
      grepl("fine", ., ignore.case = TRUE) | grepl("^\\s*F\\s*$", ., ignore.case = TRUE) ~ "fines", # match 'fine' or single 'F'
      TRUE ~ .
    ))
  ) %>%
  
  # Step 2 — create Embedded_* columns from entries with a numeric value and uppercase 'E', assign 'free' to non embedded pieces
  mutate(across(
    starts_with("B_Axis_T"),
    ~ ifelse(grepl("E", ., ignore.case = FALSE), "E", "free"),
    .names = "Embedded_{.col}"
  )) %>%
  
  # Step 3 — remove 'E' from numeric entries
  mutate(across(
    starts_with("B_Axis_T"),
    ~ gsub("\\s*E\\s*", "", ., ignore.case = TRUE)
  )) %>%
  
  # Step 4 — convert purely numeric strings to numeric type
  mutate(across(
    starts_with("B_Axis_T"),
...
Let's take a brief look through the dataframe. Look out for any errors and explore unusual entries. Note that, if you recorded embedded pebbles, there will be new columns that indicate if the pebble is embedded or 'free'
...
datatable(
  results_df %>%
    ...
    dplyr::select(Site, Year, `Size class / mm`, `Cumulative proportion`, `Count`),  # Reorder and rename only in display
  rownames = FALSE,
  options = list(
    scrollX = TRUE,  # Enable horizontal scrolling
    scrollY = "500px",  # Set the height of the scrollable window
...
D50_values <- my.Dvalues %>%
 filter(percentile == 50) %>%
  dplyr::select(Site, Year, D50 = estimate)  # Rename column for clarity
...
#       `Count` = counts
#     ) %>%
#     dplyr::select(Site, Time_Category, `Size class / mm`, `Cumulative proportion`, `Count`),  # Reorder and rename only in display
#   rownames = FALSE,
#   options = list(
#     scrollX = TRUE,  # Enable horizontal scrolling
#     scrollY = "500px",  # Set the height of the scrollable window
