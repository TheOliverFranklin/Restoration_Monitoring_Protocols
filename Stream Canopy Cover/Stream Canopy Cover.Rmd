...
data_dir <- "C:/Users/franklino/Documents/Restoration_Monitoring_Protocols/data_raw/Stream Canopy/to_combine"
```
...
This code generates a plot that illustrates canopy cover across transects and along the surveyed area. To ensure the plot is oriented intuitively, please indicate whether transect 1 is at the upstream or downstream extent of the survey, by editing the below code chunk. 

Note that you must follow Steps 1 and 2 in the below code if you have only one year of data, or if your survey direction was consistent across all sites and years that will be analysed today. If your survey direction was not consistent across all site years, complete Steps 1 and 3 in the below code.

```{r survey direction}
# User input required:

# ---------- Step 1: change the below 'TRUE' to 'FALSE' if the direction of your survey was not consistent across all sites and years analysed in this session
consistent_direction <- TRUE   # If this remains 'TRUE', specify the direction at Step 2. If you enter 'FALSE' go to Step 3.

if (consistent_direction) {
  global_whereisT1 <- "downstream"   # ----------  Step 2: specify whether transect 1 is "upstream" or "downstream" (include quotation marks). Then there are no more user inputs for you in this section.
  
  survey_direction <- df %>%
    distinct(Site, year) %>%
    mutate(Transect1Location = global_whereisT1)
  
} else {
  # ----------  Step 3: List all site-year combinations in the data to be analysed in this session, as well as the location of Transect 1 as "upstream" or "downstream"  (include quotation marks)
  
  survey_direction <- tibble::tribble(
    ~Site,   ~year, ~Transect1Location,
    "Mayse", 2024,  "upstream",
    "Mayse", 2025,  "upstream",
    "Raven Left", 2024,  "downstream",
    "Raven Left", 2025,  "upstream"
    # add more rows for extra Site-years, and dont forget the comma for the penultimate line
  )
}
```
...
At this point, when running the code in RStudio, you will be prompted to specify a folder for the outputs (figures, tables etc.). We recommend a dedicated folder that will contain all outputs from this session, so that they can be easily distinguished from other sites or years in which data is analysed. When prompted you will need to enter the folder location, without any quotation marks 
(e.g., C:/Documents/Restoration_Monitoring_Protocols/data_tidier/Site_Date). 
...
if (interactive()) {
  ...
  output_folder <- "C:/Users/franklino/Documents/Restoration_Monitoring_Protocols/data_tidier/StreamCanopy"
  if (!dir.exists(output_folder)) {
    dir.create(output_folder, recursive = TRUE)
...
Note that the transects in this figure are oriented based on your previously-specified survey direction, and are displayed with upstream at the bottom and downstream at the top. For example, if at site A your 'Transect 1' was downstream, it will be at the top of the plot.
...
# add survey_direction table so each Site-year has intuitive direction
df2 <- df %>%
  left_join(survey_direction, by = c("Site", "year")) %>%
 group_by(Site, year) %>%
  mutate(
    max_transect_no = max(as.numeric(as.character(Transect_No.)), na.rm = TRUE),
    Transect_plot_order = if (first(Transect1Location) == "upstream") {
      as.numeric(as.character(Transect_No.))
    } else {
      max_transect_no - as.numeric(as.character(Transect_No.)) + 1
    }
  ) %>%
  ungroup()
...
heatmap_plot <- ggplot(df_long, aes(x = Position, y = Transect_plot_order, fill = Percent_Cover)) +
  ...
  scale_y_continuous(expand = expansion(mult = c(0, 0)),breaks = NULL,labels = NULL) + # suppress y axis labels (incompatible if surveys conducted in different directions)
  labs(x = "Stream Position", y = expression("Transect Order: Upstream " %->% " Downstream"), title = "Stream Canopy Cover by \nSpherical Densiometer Measure") +
  theme_minimal() +
