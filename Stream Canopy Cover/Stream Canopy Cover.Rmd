...
This code generates a plot that illustrates canopy cover across transects and along the surveyed area. To ensure the plot is oriented correctly, please indicate whether transect 1 is at the upstream or downstream extent of the survey, by editing the below code chunk:

```{r survey direction}
# --- User input: specify whether Transect 1 is upstream or downstream ---
Transect1Location <- "upstream"   # options: "upstream" or "downstream" (include quotation marks)
```
...
# Set the order of Transect_No. depending on prior input
if (Transect1Location == "upstream") {
  transect_levels <- as.character(seq(1, max_transect_no, 1))   # 1 at top
} else if (Transect1Location == "downstream") {
  transect_levels <- as.character(seq(max_transect_no, 1, -1))  # 1 at bottom
} else {
  stop("Transect1Location must be 'upstream' or 'downstream'")
}

# Convert Transect_No. back to a factor with full range for consistent y-axis
df <- df %>%
 mutate(Transect_No. = factor(Transect_No., levels = transect_levels))
