...
We have written this script in R markdown (.rmd file) and printed it as an .html document. As **Step 1** we recommend that you read through the .html file, which we printed using some fabricated example data. The .html file contains all the written guidance, but omits most of the underlying code. **Step 2** is to then open this file in both .html and .rmd formats, and begin processing your own data:
...
This script is adapted from Bellucci et al. 2020, in which they developed and tested a methodology that assesses longitudinal stream connectivity using trail cameras. Their approach involves the following:

1. Assigning stream connectivity categories (see Table 1, below) to hourly trail camera images
2. Calculating average daily stream connectivity estimates from the hourly images. 
3. Generating summary metrics using the daily estimates. The metrics are used to examine variability in stream connectivity across the monitoring period (Table 3). 
...
We adapted their approach for the context of monitoring restoration projects, and introduced the categorical rating system for application at nodes (confluences or bifurcations with off-channel or side-channel habitats). In adapting the approach for restoration monitoring, we have assumed a BACI (before-after-control-impact) study design, but the code will still be useful to examine your data in the absence of multiple years and/or without control sites. Notably, the inferential test we included is designed for data with both before/after and with a control or reference site. Without this data it is not possible to statistically control for background variation and to therefore attribute observed changes to their restoration actions.
...
**StreamTrackR**: An R tool for managing and analyzing stream monitoring data, helping users track sites, access up to date hydrometric information, and analyze trends. It generates summary reports of current streamflow conditions at selected stations, outputting the results in a tidy HTML format.
...
Table 1 presents the six stream connectivity categories developed by Bellucci et al. 2020 for trailcam images collected at riffle-pool sequences. Beneath this, in Table 2, are the corresponding categories that we propose to use for images collected at nodes.
...
Before we can generate the metrics, you must assign a category from either Table 1 or Table 2 (as applicable) to each hourly trailcam photo in which the stream is visible. 
...
When categorising the data, you should create a csv file with columns featuring the reference name for the reach/location (call this column *'Site'*), date (*'Date'*), and the rating for each image (column name *'Obs'*). It is not necessary to record a rating for every single hourly image (e.g., if there is insufficient light for a determination). This script will calculate the average daily rating from the hourly ratings that you enter. You do not need to specify the time for each image, as our code will just use the date to generate the average daily rating.
...
In the code chunk below, we prepare the data for analysis. You may need to edit a couple of values in the code to suit your project. In particular:

-  Depending on the formatting of your date, *you may need to adjust the code below to match your date format* (e.g., change the part that says 'mdy' to ydm, dmy, myd, dym, etc.)

-  You may need to filter the data to include only the range of interest. To do this, *adjust 'filter(Month %in% 7:10)' as necessary*. In this instance, we have filtered the data to only include months from July (7th month) through to October (10th month) inclusive.
...
We can now take a look at the data, and check that everything looks as it should. The column DOY refers to day of year (e.g., the 182nd day of the year is July 7th), avg_Obs refers to the mean of the hourly ratings for that day, rounded to the nearest integer, and DurObs is a binary (0/1) assignment used by Bellucci et al 2020 which groups categories 1-3 as disconnected and categories 4-6 as connected.
...
We will export the tidied dataframe to your specified folder for reference or subsequent analyses.
...
Bellucci et al. 2020 developed metrics to describe the magnitude, frequency, duration, and timing components of stream connectivity (Table 3) based on the average daily stream connectivity ratings (Table 1). The metrics were developed to compare locations on the same stream (e.g., upstream and downstream of a stressor), or to compare a potentially impacted site to a least disturbed site. 
...
Table 4 below presents the above stream connectivity metrics based on the average daily stream connectivity ratings in your data and grouped by site and by year. We export both tables 3 and 4 to your output folder for future reference.
...
Here we present two figures that should be useful in exploring changes at your restoration site and at the control site over time. The figures show how the daily rating changes through time along the x axis (day of year).
...
However, if you are comfortable with this, the generalized linear model below can be useful in teasing apart changes due to restoration actions, background changes in seasonality of flows, and unmeasured differences among sites.
...
Enter in the below code the years that you consider 'before' and 'after' your restoration action. You may have more than one year in each category. It is important that the data from reference and restoration sites are aligned temporally. 
...
- How the daily rating values may have changed differently between the control and reference sites since the restoration action (site x time), as well as
- How the rate of change of the daily rating values through the season may have changed differently between the control and reference sites since the restoration action (site x time x day of year). 

As an example of the difference between the above two interactions: In the former we might detect an absolute increase in average daily ratings at our restoration site after the restoration (and this was not observed at our reference site); In the latter we might, despite perhaps the same average daily rating, see at our restoration site (and after restoration) that the daily ratings stay higher for longer before falling, and that this change was not evident at the reference site. You may understand this graphically as a shift in the elevation of a line, versus a change in the shape of a line.

Other interaction terms can tell us whether there are potential background differences between the seasonal flow patterns between sites (site x day of year), or whether the seasonal flow patterns appear to have changed over the years, similarly at both sites (time x day of year).
...
Here are a few notes about our specific approach:
...
- This model is useful for interpreting whether or not there are effects that appear 'real' or statistically significant (i.e., confidence intervals do not span zero and are not excessively broad) and in understanding the direction of the effect (negatively or positively effecting the rating value). The estimated effect sizes for each coefficient are *not* considered comparable or easily interpretable because they use different scales (e.g., factor, quadratic) and the transformations and interactions generally preclude interpretation of these values. Your interpretation of the coefficients should be qualitative in nature. For this reason, we do not plot or export a coefficient figure comparing the effect size estimates, though we do provide the code in the below chunk.
...
Considering these results together, we observed changes that are not associated with our restoration action (sites had underlying differences in seasonal rating trends, as well as both trends changing in a similar way over the years). However, despite these outside influences on the daily ratings, our restoration site was uniquely associated with an increase in the average value of ratings as well as an altered seasonal trend, which we can consider attributable to our restoration actions. The simplest way to better understand this trend is to look at the metrics and the figures generated previously, which imply that our restoration caused an increase in connectivity and that higher levels of connectivity persisted later into summer, before returning to levels similar to the reference site.
