...
pacman::p_load(rmarkdown, ggplot2,lubridate, dplyr, stringr, tidyr, purrr, reactable, knitr, htmltools, tmap, sf,leaflet,kableExtra, DT, update=F) # Note, update=F prevents pacman from checking for package updates when loading them. Useful to avoid unexpected version changes and slow script execution. Adjust as necessary. 
```
...
# Prepare data
data <- df %>%
  mutate(Date = dmy(Date),  # Convert 'Date' column to Date class
         Year =year(Date),
         Month = month(Date),  # 
         DOY = yday(Date)) %>%  # 
...
- **Rating(<sub>t-1</sub>)** is a lag term to account for autocorrelation, it uses the previous day's rating as an explanatory variable.
...
As an example of the difference between these two interactions: In the former we might detect an absolute increase in average daily ratings at our restoration site after the restoration that was not observed at our reference site; In the latter we might, despite perhaps the same average daily rating, see at our restoration site (and after restoration) that the daily ratings stay higher for longer before falling, and that this change was not evident at the reference site.

Additional interaction terms can tell us whether there are potential background differences between the seasonal flow patterns between sites (site * day of year), or whether the seasonal flow patterns appear to have changed over the years, similarly at both sites (time * day of year).
