...
pacman::p_load(rmarkdown, ggplot2,lubridate, dplyr, stringr, tidyr, purrr, reactable, knitr, htmltools, tmap, sf,leaflet,kableExtra, DT, MASS, car, DHARMa, glmmTMB, boot, update=F) # Note, update=F prevents pacman from checking for package updates when loading them. Useful to avoid unexpected version changes and slow script execution. Adjust as necessary. 
```
...
-  Convert the 'Date' column (character data) to date class. Depending on the formatting of your date, *you may need to adjust the code below to match your date format* (e.g., change the part that says 'dmy' to ydm, mdy, myd, dym, etc.)
...
# Prepare data
data <- df %>%
  mutate(Date = str_remove(Date, " .*"),  # remove everything after the first space (e.g., sometimes timestamp is here after space)
         Date = mdy(Date),  # Convert 'Date' column to Date class
         Year =year(Date),
         Month = month(Date),  # 
         DOY = yday(Date)) %>%  # 
  filter(Month %in% 7:10) %>%  # Filter for months of interest
  group_by(Site,Date, Year, Month, DOY) %>%  # 
  summarize(avg_Obs = round(mean(Obs, na.rm = TRUE)),  # daily rating consistent with Bellucci et al 
            MeanRating = mean(Obs, na.rm = TRUE), # mean (not rounded) daily rating for use in inferential model
...
Table 4 below presents the numerical results for all of the above stream connectivity metrics based on the average daily stream connectivity categories in your data.
...
flow_metrics_df <- data %>%
  ...
  unnest(metrics) %>% 
  arrange(Site, Metric) %>% # arrange in alpha order
  pivot_wider(
...
#export table as needed
export_table(as.data.frame(flow_metrics_df), "flow_metrics.csv")
...
Enter in the below code the years that you consider 'before' and 'after' your restoration action. You may have more than one year in each category. It is important that the data from reference and restoration sites should be aligned temporally. 
...
- We generate confidence intervals, and recommend interpretations based on confidence intervals rather than p-value cut-offs. We are using the same data to generate a dispersion parameter and to estimate model coefficients. With smaller sample sizes (in the hundreds) this can be anti-conservative (more false positives) and confidence intervals can better reflect the uncertainty in estimates.

# perhaps a note/ fix for if you have discontinuous data (e.g. if your battery ran out and you have a gap in your season... the lag variable would presumably assume that the day before the break is impacting the day after in the same way...)

# model_nb <- glm.nb(MeanRating ~ Site * Time_Category * poly(DOY, 2) + MeanRating_lag,  data = data_model
# poisson_model <- glm(MeanRating ~ Site * Time_Category * poly(DOY, 2) + MeanRating_lag, family = poisson, data = data_model)
# data may actually be underdispersed. rounding of daily value could contribute to this. so I should create a new variable (MeanRating) and use this in the glm. 
# I should test dispersion first and use appropriate model (nb for overdispersion, poisson for dispersion around 1, underdispersed could try COM-Poisson?? or quasi-Poisson?) Maybe I should try COM-Poisson given the variety of datasets and its flexibilty, and the high potential for underdispersed data (many day values at the same level through summer)

# I want to use CIs and not rely on p values, since COM-Poission uses the same data to generate a dispersion parameter as to estimate the model coefficients, which can be anti-conservative  (increased type 1 errors) if we have small sample sizes (which may be the case if we have only a few months of daily data)

#  Ishould include example output and interpretation.

# also include predicted seasonal curves?
  
```{r glm, echo = FALSE}

# sort data to create lag variable within each site-year
data <- data %>%
  arrange(Site, Year, DOY) %>%
  group_by(Site, Year) %>%
  mutate(avg_Obs_lag = lag(avg_Obs)) %>%
  ungroup()
...
# generate confidence intervals (profile likelihood)
confint(model_com, method = "profile")
```
...
- inferential test
- ensure exports are correct (all figs, and table with descrtiptiuonsof metric,s and metrics)
- test with actual raw daily data
- remove MeanRating if it is not used (COM Poisson needs count)
